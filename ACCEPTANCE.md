...
本次完成 Step 7 的低风险 SEO 基础改造：移除 `Create Next App` 默认 metadata，为核心用户端路由提供独立标题和描述，并避免直接在 `"use client"` page 中导出 metadata。

本次完成范围：

1. 更新全局 `app/layout.tsx` metadata。
2. 为 `/books` 添加独立 metadata。
3. 为 `/books/[id]` 添加独立静态 metadata。
4. 为 `/favorites` 添加独立 metadata。
5. 为 `/dashboard` 添加独立 metadata。
6. 为 `/pricing` 添加独立 metadata。
...
- `title.default: "BookWise"`
- `title.template: "%s | BookWise"`
- `description` 改为 BookWise 项目描述
- 新增：
  - `applicationName`
  - `keywords`
  - `authors`
...
- 浏览器标签页不再使用 `Create Next App`。
- 没有单独 metadata 的页面会使用 `BookWise` 作为默认标题。
- 子路由 title 会通过 template 生成 `Page Title | BookWise`。

---

#### 2. 核心页面 metadata

当前核心用户端页面大多是 `"use client"` page，不能直接在 page 文件里导出 metadata。

为避免大范围移动页面代码，本次采用 Next.js 支持的 server `layout.tsx` 方案：在对应路由段新增 layout，由 server layout 承载 metadata，原 client page 保持不动。

新增文件：

```ts
app/(user)/books/layout.tsx
app/(user)/books/[id]/layout.tsx
app/(user)/favorites/layout.tsx
app/(user)/dashboard/layout.tsx
app/(user)/pricing/layout.tsx
```
...
- 本次只做静态详情页标题。
- 动态书名、description 截断、OG image 需要把当前大型 client detail page 拆成 server page + client component。
- 该拆分会触及音频、评论、收藏等大量交互逻辑，风险更高，按计划保留到后续详情页拆分阶段。
...
```ts
My Favorites | BookWise
```

描述：

```ts
Review and manage your saved BookWise books in one place.
```
...
```ts
Track your BookWise subscription, listening progress, and learning activity.
```
...
本次没有实现 `/books/[id]` 的动态 metadata。

原因：

- 当前 `app/(user)/books/[id]/page.tsx` 是大型 Client Component。
- 动态 metadata 需要把 page 改成 Server Component，再把现有交互逻辑移动到 `BookDetailClient.tsx`。
- 这会提前执行 Step 11 的“大文件拆分”工作，并且涉及音频、收藏、评论多个交互状态。
- 为降低风险，本次只给详情页提供独立静态标题，满足 Step 7 的最低验收要求。

本次没有给 `/pricing/checkout` 单独添加 metadata。

原因：

- Step 7 核心页面列表要求的是 `/pricing`。
- checkout 会继承 `/pricing` layout 的 metadata。
- 后续如果 checkout 需要独立分享或标题，可以单独加 checkout layout。
...
- 新增 server layout metadata 没有和现有 client pages 冲突。
- production build 完整通过。
- 当前仍有 Next.js warning：`middleware` 文件约定已 deprecated，建议后续迁移到 `proxy`，不阻塞构建。
...
- 新增 metadata layout 文件没有出现在 lint 错误列表中。
- 剩余 lint 失败仍是既有债务，集中在：
  - admin 端 `any`
  - admin/dashboard 内部路由使用 `<a>`
  - admin/users、user/dashboard、pricing 的 `react-hooks/immutability`
  - 多处 `<img>` 未替换为 `next/image`
...
1. `/books/[id]` 目前只有静态 `Book Details` 标题，尚未根据书名生成动态 metadata。
2. 当前未做浏览器手动检查标签页标题，只通过 Next build 验证 metadata 写法合法。
3. `middleware` deprecated warning 仍存在，后续可按 Next.js 16 建议迁移到 `proxy`。

下一步建议：

1. 进入 Step 9，优化用户端分页体验。
2. 在 Step 11 或详情页拆分阶段，把 `/books/[id]` 拆成 Server Page + Client Component，再补动态书名、description 和 OG image。
3. 后续用浏览器手动验证 `/books`、`/favorites`、`/dashboard`、`/pricing`、`/books/:id` 的标签页标题。

### 验收标准

- [x] 浏览器标签页不再显示 `Create Next App`（全局 metadata 已替换）。
- [x] 每个核心页面标题唯一（books/favorites/dashboard/pricing/detail 均有独立 title）。
- [x] 图书详情页至少有独立标题；动态书名和封面分享在 Server/Client 拆分完成后验收。

### 阶段结论

**Step 7 可判定为基础 SEO metadata 实现完成。**

理由：

- 默认 `Create Next App` metadata 已移除。
- 核心用户端页面已获得独立标题和描述。
- 使用 server layout 承载 metadata，避免违反 Client Page 规则。
- production build 通过。
- 动态图书详情 metadata 已明确留到详情页 Server/Client 拆分时处理。
